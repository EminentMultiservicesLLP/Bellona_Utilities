...
SET @INVOICEDAY = '2024-11-23';
--select top 10 * from Rista_SaleInvoices
SELECT COUNT(DISTINCT R.branchCode) branches FROM Rista_SaleInvoices(nolock) R WHERE InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM' AND branchCode = 'MUM';

select count(distinct invoiceid) Invoices FROM Rista_SaleInvoices (nolock) WHERE InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM';

select count( distinct a.invoiceid) SalesItems FROM Rista_SaleItems (NOLOCK) A 
INNER JOIN Rista_SaleInvoices (NOLOCK) B ON   a.InvoiceID = B.InvoiceID AND B.InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM';
...
INNER JOIN Rista_SaleInvoices C ON B.InvoiceID = C.InvoiceID AND C.InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM';
...
INNER JOIN Rista_SaleInvoices D ON C.InvoiceID = D.InvoiceID AND D.InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM';
...
INNER JOIN Rista_SaleInvoices (NOLOCK) B ON   a.InvoiceID = B.InvoiceID AND B.InvoiceDay = @INVOICEDAY  AND b.branchCode = 'MUM';
...
WHERE A.InvoiceDay = @INVOICEDAY  AND branchCode = 'MUM' AND B.InvoiceID IS NULL AND InvoiceType <> 'NC' AND Status <> 'CANCELLED'  AND A.NetAmount > 0
*/